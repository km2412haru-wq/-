...
// 全問回答済みになってから結果画面へ切り替わるまでの間（選択した内容が
// 目に見える一瞬の間を置くための遅延）。
const AUTO_FINISH_DELAY_MS = 400
...
export default function Exam() {
  ...
  const [answers, setAnswers] = useState([]) // 選択したchoiceIndex、未回答はnull
  ...
  // 全問回答し終えたら、少し間を置いて自動的に結果画面へ
  useEffect(() => {
    if (!started || answers.length === 0) return
    const allAnswered = answers.every((a) => a !== null)
    if (allAnswered) {
      autoFinishRef.current = setTimeout(handleSubmit, AUTO_FINISH_DELAY_MS)
      return () => clearTimeout(autoFinishRef.current)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [answers, started])
  ...
  function handleSubmit() {
    if (submittedRef.current) return // タイムアップと全問回答が同時に発火するのを防ぐ
  ...
  if (!started) {
    return (
      <div className="mx-auto max-w-xl space-y-4 rounded-lg border border-slate-200 bg-white p-8 text-center">
        <h1 className="text-2xl font-bold">③ 模擬試験（ミニ版）— {exam.code}</h1>
        ...
        <ul className="mx-auto max-w-sm list-disc space-y-1 pl-5 text-left text-sm text-slate-500">
          <li>問題プールからランダムに出題されます</li>
          <li>試験中は正誤・解説を表示しません</li>
          <li>全問回答し終えるか10分経過すると、自動的に結果画面へ移ります</li>
          <li>未回答のまま終了した問題は不正解として扱われます</li>
        </ul>
        <button
          onClick={startExam}
