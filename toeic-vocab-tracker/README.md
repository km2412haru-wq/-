...
- **単語登録**: 単語(英語)・意味(日本語)・品詞を入力して登録。登録日時は自動記録、「定着/未定着」のチェック状態を持つ
- **意味の予測変換**: 単語入力が止まると、Claudeが予測した意味を意味欄のplaceholderに薄く表示(タップで入力欄に反映)。意味欄に自分で入力し始めると止まる。無駄打ちを避けるため入力停止後600msのデバウンスあり
- **発音記号(アクセント)表示**: 「🔤 アクセント表示」でClaudeがIPA発音記号(第一強勢の位置を含む)を生成。手動編集も可能
- **発音読み上げ**: 単語・例文の🔊ボタンでブラウザ標準のWeb Speech APIを使って読み上げ(API課金なし、完全無料)
...
- **例文の自動生成**: 単語・熟語のカードから「📝 例文を生成」でClaudeがTOEICのビジネスシーンを想定した英語例文+日本語訳を作成。再生成・手動編集に加えて、カードごとに「✕」で表示/非表示を切り替え可能
- **一覧・検索**: カード形式の一覧表示(品詞ごとに色分け、熟語は専用の配色)、単語名での検索、並び替え(登録日順/アルファベット順)、定着/未定着・単語/熟語での絞り込み
...
APIキーが未設定でも、単語の登録・一覧・検索・チェック操作(すべてlocalStorageベース)はそのまま使えます。派生語の自動提案・例文生成・意味予測機能を使う場合のみAPIキーが必要です。

> **注意**: 単語データは各ユーザーのブラウザの`localStorage`にのみ保存されるため、同じURLを他の人と共有してもお互いのデータは見えません。ただしAI機能(派生語提案・例文生成・意味予測)は全員が同じ`ANTHROPIC_API_KEY`を共有して呼び出すため、利用量に応じた課金は全員分がこのキーの持ち主に集約されます。
...
app/
  ...
  api/generate-phonetic/          発音記号生成のAPI Route(同上)
  api/suggest-meaning/            意味予測のAPI Route(同上。入力デバウンスして呼び出し回数を抑制)
components/
  ...
  SpeakButton.tsx                   Web Speech APIによる発音読み上げボタン
lib/
  ...
  speech.ts                         Web Speech APIラッパー(読み上げ)
types/word.ts                      型定義
```
