...
- **発音記号(アクセント)表示**: 単語・熟語の登録時、および派生語を採用した時にClaudeがIPA発音記号(第一強勢の位置を含む)を自動生成し、単語名の下に常時表示(専用ボタンは無し)。手動編集は可能
- **語法メモ**: 「📌 語法メモを生成」で、相性の良い前置詞(depend on 等のコロケーション)・後置修飾の形容詞用法・特徴的な言い回し(定型表現)・可算/不可算や自動詞/他動詞の区別など、TOEIC Part 5で問われやすい語法上の特徴をClaudeが解説。該当が無ければ「特記事項はありません」。手動編集・カードごとの表示/非表示切り替えに対応
...
app/
  ...
  api/generate-usage-note/        語法メモ生成のAPI Route(同上)
