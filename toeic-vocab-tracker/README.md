...
- **派生語の自動提案**: 単語登録時にClaudeが派生語候補を提案。ユーザーは候補ごとに意味・品詞を編集した上で、個別に採用/却下できる
- **一覧・検索**: カード形式の一覧表示(品詞ごとに色分け)、単語名での検索、並び替え(登録日順/アルファベット順)、覚えた/未定着での絞り込み
- **グループ表示**: 元の単語とその派生語をグループ単位でまとめて表示する切り替えビュー
- **データ永続化**: 単語データはブラウザの`localStorage`にのみ保存(サーバー/DBなし)。オフラインでも一覧の閲覧・チェック操作が可能。派生語の自動提案にはネット接続が必要
...
- [Anthropic Claude API](https://docs.anthropic.com/)(`@anthropic-ai/sdk`。tool useで派生語候補をJSON形式に固定して取得)
...
app/
  ...
  api/suggest-derivatives/        Claude呼び出しAPI Route(APIキーはサーバー側のみで使用)
components/
