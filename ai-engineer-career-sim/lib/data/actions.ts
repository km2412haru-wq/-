...
export const ACTIONS: GameAction[] = [
  {
    ...
    category: "planning",
    phaseRequired: "data",
    tooltip: "認識のズレを早めに解消し、後工程の手戻りリスクを減らす。地味だが最重要の一手。「データ収集・分析」フェーズの一手。",
  ...
  },
  {
    ...
    category: "data",
    phaseRequired: "data",
    tooltip: "モデルの土台となるデータを整える。「質」を取るか「量」を取るかのトレードオフ。「データ収集・分析」フェーズの一手。",
  ...
  },
  {
    ...
    category: "model",
    phaseRequired: "model",
    tooltip: "自社学習・API利用・OSSファインチューニングから戦略を選ぶ。コストと自由度のトレードオフ。「AIモデル開発」フェーズの一手。",
  ...
  },
  {
    ...
    category: "build",
    phaseRequired: "model",
    tooltip: "手を動かして動くものを作る。工数を大きく消費する代わりに進捗が大きく進む。「AIモデル開発」フェーズの一手。",
  ...
  },
  {
    ...
    category: "build",
    phaseRequired: "model",
    tooltip: "プロンプトの工夫や外部知識検索（RAG）で回答精度を底上げする。「AIモデル開発」フェーズの一手。",
  ...
  },
  {
    ...
    apCost: 1,
    category: "build",
    phaseRequired: "implementation",
    tooltip: "開発したモデルを実際の業務システムに組み込む。Web API化・オンプレ・クラウド推論・エッジデバイスから方式を選ぶ。「システムへの実装」フェーズの一手。",
    term: { name: "推論", desc: "学習済みのモデルに新しい入力を渡し、予測・分類などの結果を出力させること。" },
    choices: [
      {
        id: "web_api",
        label: "Web APIとして公開する（バランス型）",
        tooltip: "外部から呼び出せる形にする定番の方式。モデルの更新や差し替えがしやすい。",
        when: "特にクセがなく、バランス良く進めたい時",
        effects: [
        ...
        apply: (s) => ({
          state: {
            ...s,
            progress: clamp(s.progress + Math.round(8 * familiarityFactor(s)), 0, 100),
            budget: s.budget - 15,
            fatigue: clamp(s.fatigue + 4),
            reputation: gainReputation(s, 2),
            ...bumpCombo(s),
          },
          log: "モデルをWeb API化した。今後の差し替えもしやすくなった。",
        }),
      },
      {
        id: "onprem",
        label: "オンプレミスに実装する（高セキュリティ）",
        tooltip: "社内ネットワーク内で稼働させる。コストと時間はかかるが信頼性が高い。",
        when: "セキュリティ要件が厳しい案件、精度・信頼性を重視したい時",
        effects: [
          { label: "精度+6", tone: "good" },
          { label: "進捗+4", tone: "good" },
          { label: "評価+2", tone: "good" },
          { label: "予算-30万円", tone: "bad" },
          { label: "疲労+6", tone: "bad" },
        ],
        apply: (s) => ({
          state: {
            ...s,
            quality: clamp(s.quality + 6),
            progress: clamp(s.progress + Math.round(4 * familiarityFactor(s)), 0, 100),
            budget: s.budget - 30,
          ...
          log: "オンプレミス環境に実装した。時間はかかったが信頼性の高いシステムになった。",
        }),
      },
      {
        id: "cloud_inference",
        label: "クラウド推論サービスを使う（スケーラブル）",
        tooltip: "AWS/Azure/GCPなどのAI推論基盤を利用する。立ち上げが早くスケールしやすい。",
        when: "スピード重視で、将来の利用増にも備えたい時",
        effects: [
          { label: "進捗+11", tone: "good" },
          { label: "評価+1", tone: "good" },
        ...
        apply: (s) => ({
          state: {
            ...s,
            progress: clamp(s.progress + Math.round(11 * familiarityFactor(s)), 0, 100),
            budget: s.budget - 12,
            fatigue: clamp(s.fatigue + 3),
            reputation: gainReputation(s, 1),
            ...bumpCombo(s),
          },
          log: "クラウド推論サービスを使って実装した。立ち上げが速く、スケールにも強い。",
        }),
      },
      {
        id: "edge",
        label: "エッジデバイスに組み込む（リアルタイム向け）",
        tooltip: "工場設備やカメラなど、現場でリアルタイムに推論させる。難度は高いが技術力が大きく伸びる。",
        when: "現場でのリアルタイム性が求められる案件、技術力を大きく伸ばしたい時",
        effects: [
          { label: "技術力+6", tone: "good" },
          { label: "進捗+5", tone: "good" },
          { label: "評価+2", tone: "good" },
          { label: "予算-20万円", tone: "bad" },
          { label: "疲労+7", tone: "bad" },
        ],
        apply: (s) => ({
          state: {
            ...s,
            techScore: s.techScore + 6,
            progress: clamp(s.progress + Math.round(5 * familiarityFactor(s)), 0, 100),
            budget: s.budget - 20,
            fatigue: clamp(s.fatigue + 7),
            reputation: gainReputation(s, 2),
            ...bumpCombo(s),
          },
          log: "エッジデバイスへの組み込みに挑戦した。難しかったが技術力が大きく伸びた。",
        }),
      },
    ],
  },
  {
    id: "operation_improve",
    label: "運用しながら改善する",
    emoji: "🔄",
    apCost: 1,
    category: "quality",
    phaseRequired: "operation",
    tooltip: "実装したあとも精度を確認し続け、必要に応じて再学習・改善を行う。「運用・改善」フェーズの一手。",
    term: { name: "MLOps", desc: "Machine Learning Operations。学習・評価・デプロイを一連の流れとして管理し、モデルを継続的に改善していく考え方。" },
    choices: [
      {
        id: "retrain",
        label: "新しいデータで再学習する",
        tooltip: "入力データの傾向の変化に合わせてモデルを更新し、精度を保つ。",
        when: "精度の劣化が心配な時",
        effects: [
          { label: "精度+8", tone: "good" },
          { label: "進捗+5", tone: "good" },
          { label: "評価+2", tone: "good" },
          { label: "予算-15万円", tone: "bad" },
          { label: "疲労+4", tone: "bad" },
        ...
        apply: (s) => ({
          state: {
            ...s,
            quality: clamp(s.quality + Math.round(8 * familiarityFactor(s) * techBonus(s))),
            progress: clamp(s.progress + Math.round(5 * familiarityFactor(s)), 0, 100),
            budget: s.budget - 15,
            fatigue: clamp(s.fatigue + 4),
            reputation: gainReputation(s, 2),
          ...
          log: "新しいデータでモデルを再学習した。精度の劣化を防げた。",
        }),
      },
      {
        id: "monitor",
        label: "監視体制を整える",
        tooltip: "異常やトラブルの兆候を早期に検知できる仕組みを作る。",
        when: "トラブル発生率を下げたい時",
        effects: [
          { label: "トラブル発生率-12", tone: "good" },
          { label: "評価+2", tone: "good" },
          { label: "予算-8万円", tone: "bad" },
          { label: "疲労+3", tone: "bad" },
        ],
        apply: (s) => ({
          state: {
            ...s,
            riskLevel: clamp(s.riskLevel - 12, 0, 100),
            budget: s.budget - 8,
            fatigue: clamp(s.fatigue + 3),
            reputation: gainReputation(s, 2),
            ...bumpCombo(s),
          },
          log: "運用の監視体制を整えた。異常があってもすぐに気づけるようになった。",
        }),
      },
      {
        id: "docs",
        label: "運用ドキュメント・引き継ぎ資料を整備する",
        tooltip: "誰が運用を引き継いでも困らないよう、資料としてナレッジを残す。",
        when: "満足度やコミュ力を伸ばしたい時",
        effects: [
          { label: "満足度+8", tone: "good" },
        ...
        apply: (s) => ({
          ...
          log: "運用ドキュメントを整備した。誰でも引き継げる体制になった。",
        }),
      },
    ],
  },
  {
    ...
    category: "quality",
    phaseRequired: "operation",
    tooltip: "モデルの弱点を洗い出す。早期にバグを発見できることもある。「運用・改善」フェーズで継続的に行う一手。",
  ...
  },
  {
    ...
    category: "quality",
    phaseRequired: "implementation",
    tooltip: "品質は上がるが、稀にレビューでバグが発覚し手戻りが発生する。「システムへの実装」フェーズの一手。",
