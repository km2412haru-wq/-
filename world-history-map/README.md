...
`lib/wikipedia.ts` の記事検索・フォールバック・キャッシュロジック、見出しの時代分類（`classifyEra`）、記事本文のセクション分割（`parseSections`）、時代ごとのグルーピング（`groupIntoEras`）を、ローカルに立てたHTTPサーバー上のフィクスチャ・ユニットテストで検証しています（外部ネットワーク不要、18件）。
