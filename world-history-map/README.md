...
### 「国旗・首都・人口が不明」「関連する記事が見つからない」が続く場合

Wikipedia / REST Countries への外部リクエストが失敗している可能性があります。

- 各fetch関数は失敗時に `console.error` で詳細（HTTPステータスコードやエラー内容）を出力するようにしてあります。**Vercelのダッシュボード → 対象プロジェクト → 「Logs」（または「Runtime Logs」）タブ**を開くと、実際に何が起きているか（403で拒否されている、タイムアウトしている、等）を確認できます
- 「不明」「見つからない」という結果自体も、以前は誤って24時間キャッシュされていました（一時的な通信エラーでも丸一日その結果が固定されてしまう不具合）。現在は失敗時のキャッシュを5分に短縮したので、一時的なエラーなら数分待って再読み込みすれば直る可能性があります
- それでも改善しない場合は、Logsタブの内容を確認してみてください（REST Countries側のサービス障害である可能性もあります）
